...
function init_db($pdo) {
    ...
    try {
        ...
        if ($has_wakalah == true) {
            ...
            if (count($columns) > 0) {
                $pdo->exec("ALTER TABLE wakalah DROP COLUMN branch_name");
            }
        }
...
function seed_data($pdo) {
    ...
    $stmt_wakalah_ind = $pdo->prepare("INSERT INTO wakalah (type, name, email, phone, state, status, ic_number, address, channel) VALUES ('individual', ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt_wakalah_corp = $pdo->prepare("INSERT INTO wakalah (type, name, email, phone, state, status, company_representative, ssm_number, hq_address, address, channel) VALUES ('corporate', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    foreach ($corporates as $corp) {
        $stmt_wakalah_corp->execute([$corp[0], $corp[2], $corp[3], $corp[4], $corp[5], $corp[6], $corp[7], $corp[8], NULL, $corp[10]]);
        $wak_id = $pdo->lastInsertId();
