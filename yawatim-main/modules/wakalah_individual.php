...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($name == '' || $email == '' || $phone == '' || $ic_number == '') {
            ...
            try {
                ...
                if ($count > 0) {
                    ...
                    $stmt->execute(array($name, $email, $phone, $state, $status, $ic_number));
                    ...
                    $stmt_user->execute(array($email, $default_pass, $state, $wak_id));
        ...
        if ($id == 0 || $name == '' || $email == '' || $phone == '' || $ic_number == '') {
            ...
            try {
                ...
                if ($count > 0) {
                    ...
                    $stmt->execute(array($name, $email, $phone, $state, $status, $ic_number, $id));

                    $stmt_user = $pdo->prepare("UPDATE users SET email = ?, state = ? WHERE wakalah_id = ?");
                    $stmt_user->execute(array($email, $state, $id));
    ...
    // --- DELETE INDIVIDUAL WAKALAH ---
    if ($action == 'delete_wakalah_individual') {
        $id = 0;
        if (isset($_POST['id'])) { $id = (int)$_POST['id']; }
        ...
        if ($id == 0) {
            ...
            try {
                // Cascading delete is partially handled by DB for users.
                // We should also ensure donations are set null or handled.
                $stmt = $pdo->prepare("DELETE FROM wakalah WHERE wakalah_id = ?");
                $stmt->execute(array($id));
                set_flash('success', 'Wakalah Individual deleted successfully.');
            } catch (PDOException $e) {
                set_flash('error', 'Database error: ' . $e->getMessage());
            }
        }
...
<div class="table-controls-container" id="wakalah-individual-controls">
    <div class="search-filter-group">
        ...
        <select class="select-filter" data-filter-column="5" id="wak-ind-filter-status">
                        ...
                        <tr id="wak-ind-row-<?php echo $wak['id']; ?>">
                            ...
                            <td class="actions-td">
                                <div class="action-group">
                                    ...
                                    <form method="POST" action="index.php?page=wakalah_individual" style="display:inline;">
                                        <input type="hidden" name="action" value="delete_wakalah_individual">
                                        <input type="hidden" name="id" value="<?php echo $wak['id']; ?>">
                                        <button type="submit" class="action-btn" style="color: var(--alert-red);"
                                                title="Delete Wakalah Account"
                                                onclick="return confirm('Are you sure you want to completely delete this Wakalah Individual? This action cannot be undone.')">
    ...
    <div class="modal-container">
        ...
        <form method="POST" action="index.php?page=wakalah_individual" id="form-add-wak-ind">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="form-label" for="add_state">State Branch Office</label>
