...
// Enable emulated prepares specifically for the complex reports queries to prevent PDO Invalid Parameter Number exceptions when parameters are reused or unused
...
try {
    if ($report_type === 'donations') {
        ...
        $stmt_sum = $pdo->prepare("
            SELECT 
                COUNT(DISTINCT CASE 
                    WHEN w.type = 'individual' THEN 'Individual'
                    ELSE COALESCE(NULLIF(TRIM(d.channel), ''), 'Independent')
